...
fn_send_compile_okay_notify()
{
	...
	RTX_NOTIFY_OKAY_CONTENT="${PROJCT} ${DATE}_SVN${SVN_VERSION_NOW} 临时版本编译OK!"
...
#	No Need to save the SVN version no.
#	echo "save svn info"
...
compile_eng_new(){
	...
	if [ $EXIT != 0 ];then
		...
		RTX_NOTIFY_FAILED_INFO="${PROJCT} ${DATE}_SVN${SVN_VERSION_NOW} 临时版本编译失败！ SVN${SVN_VERSION_OLD}~${SVN_VERSION_NOW} 提交人请修正！"
